...
# Create admin and normal users with predefined passwords
sudo adduser --disabled-password --gecos "" "$admin_user"
echo "$admin_user:$admin_password" | sudo chpasswd  # Stel wachtwoord in
sudo usermod -aG sudo "$admin_user" # Root rechten

sudo adduser --disabled-password --gecos "" "$normal_user"
echo "$normal_user:$normal_password" | sudo chpasswd  # Stel wachtwoord in

# Generate SSH key pairs
sudo -u "$admin_user" ssh-keygen -t rsa -b 4096 -f "$admin_ssh_dir/id_rsa" -N ""
sudo -u "$normal_user" ssh-keygen -t rsa -b 4096 -f "$normal_ssh_dir/id_rsa" -N ""
